...
local function list_update(w, buttons, label, data, objects)
  ...
  for i, o in ipairs(objects) do
    ...
    if cache then
      ...
      tbm = wibox.container.margin(tb, dpi(4), dpi(12)) --default 4 4
    ...
    if text == nil or text == '' then
      ...
      if (textOnly:len() > 34) then --default 24 and 21
        text = text:gsub('>(.-)<', '>' .. textOnly:sub(1, 31) .. '...<')
	  ...
	  if not tb:set_markup_silently(text) then
        --tb:set_markup('<i>&lt;Invalid text&gt;</i>')
      	--TODO make focus change color (almost impossible)
		tb:set_text(textOnly:sub(1,45) .. '...' )
      end
    end
    bgb:set_bg(bg)
